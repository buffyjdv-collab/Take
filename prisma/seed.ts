...
async function main() {
  console.log('Seeding consumer-demo menu items (ConsumerMenuItem)...')
  for (const it of items) {
    await db.consumerMenuItem.upsert({
  ...
  const count = await db.consumerMenuItem.count()
  console.log(`Seed complete. ${count} consumer menu items in database.`)
}
